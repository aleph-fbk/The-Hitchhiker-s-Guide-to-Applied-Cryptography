...
\warningBlock{The \gls{ecdlp} in \gls{ecc} vs. quantum computing}{Quantum computing can solve any \gls{ecdlp} using the Shor's algorithm \cite{shor_algorithms_1994} in polynomial time (that is, efficiently) --- see \Cref{sec:TEMPORARY_LABEL:post_quantum_cryptography}.}
...
Thanks to the aforementioned properties, bilinear pairings are used for advanced cryptosystems such as \gls{ibe}, \gls{abe}, and \glspl{zkp} (\Cref{sec:identity_based_cryptography,sec:advanced_cryptography:attribute_based_encryption,sec:advanced_cryptography:zero_knowledge_proofs}, respectively). In particular, bilinearity allows for comparing results from two different sides of an equation and checking if they match --- all without revealing the secret numbers used.
